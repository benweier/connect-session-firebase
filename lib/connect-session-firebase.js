...
module.exports = function (session) {
   ...
   * @api public
   */
  var FirebaseStore = function FirebaseStore (options) {
    options = options || {};
    var db = options.database || false;
    ...
    this.sessionRef = options.sessionRef || 'sessions';

    // Initialized `firebase.database` instance.
    if (db instanceof firebase.database.Database) {
      this.db = db;
    }
    // Initialized `firebase` instance.
    else if (db instanceof firebase.app.App) {
      this.db = db.database();
    }
  ...
  FirebaseStore.prototype.get = function get (sid, fn) {
    ...
    var now = Date.now();
    var session = this.db.ref(`${this.sessionRef}/${sid}`);
  ...
  FirebaseStore.prototype.set = function set (sid, sess, fn) {
    sid = this.cleanSid(sid);
    var now = Date.now();
    var expires = typeof sess.cookie.maxAge === 'number' ? now + sess.cookie.maxAge : now + oneDayInMilliseconds;
  ...
  FirebaseStore.prototype.clear = function clear (fn) {
    var sessions = this.db.ref(this.sessionRef);
    sessions.remove(fn);
  };
