...
const connectSessionFirebase = function connectSessionFirebase (session) {
  ...
  FirebaseStore.prototype.get = function get (sid, fn) {
    ...
    session.once('value')
      .then(snapshot => {
        ...
        if (snapshot.val().expires && now > snapshot.val().expires) {
          this.destroy(sid, fn);
        } else {
  ...
  FirebaseStore.prototype.set = function set (sid, sess, fn) {
    sid = this.cleanRef(sid);
    const now = Date.now();
    const expires = typeof sess.cookie.maxAge === 'number' ? now + sess.cookie.maxAge : now + sixHoursInMilliseconds;
    const session = this.db.ref(this.sessions).child(sid);
    ...
    session.set(data).then(fn).catch(fn);
  };
  ...
  FirebaseStore.prototype.clear = function clear (fn) {
    this.db.ref(this.sessions).remove().then(fn);
  };
  ...
  FirebaseStore.prototype.reap = function reap (fn) {
    ...
    sessions.once('value')
      .then(snapshot => {
        ...
        if (!removeList.length && typeof fn === 'function') {
          return fn(null, null);
        }
